...
const receptionUploads = path.join(uploadsRoot, 'reception');
fs.mkdirSync(newsUploads, { recursive: true });
fs.mkdirSync(broadcastUploads, { recursive: true });
fs.mkdirSync(receptionUploads, { recursive: true });
...
const receptionQrUploader = multer({
  storage: multer.diskStorage({
    destination: (_req, _file, callback) => callback(null, receptionUploads),
    filename: (_req, file, callback) => callback(null, `${crypto.randomUUID()}${extensions.get(file.mimetype) || ''}`),
  }),
  limits: { fileSize: 5 * 1024 * 1024, files: 1 },
  fileFilter: (_req, file, callback) => {
...
export function uploadReceptionQrImage(req, res, next) {
  receptionQrUploader.single('media')(req, res, (error) => {
    if (!error) return next();
    if (error instanceof multer.MulterError && error.code === 'LIMIT_FILE_SIZE') {
      return next(new AppError(400, 'IMAGE_TOO_LARGE', 'Размер изображения не должен превышать 5 МБ'));
    }
