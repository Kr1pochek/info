...
- Choose — единая точка входа: `http://localhost:5174/`;
- инфокиоск: `http://localhost:5174/kiosk`;
...
Управление публикациями находится в разделе `/admin/news` существующей админ-панели. Для каждой новости редактор хранит отдельные заголовок, краткое описание и полный текст на русском и казахском языках. Там же можно выбирать категорию, загружать обложки JPG/PNG/WebP/GIF размером до 5 МБ, публиковать сразу или планировать выпуск на выбранные дату и время, снимать с публикации и удалять новости. Запланированная новость автоматически появляется в публичной ленте после наступления указанного времени.
...
Миграция `20260807090000_news` добавляет таблицу `News`, а `20260807150000_news_localization` — отдельные русские и казахские поля публикаций. Перед запуском обновлённого приложения примените миграции и при необходимости демонстрационные записи:
