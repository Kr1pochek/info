...
Публичный маршрут `/news` в режиме ожидания работает как полноэкранный корпоративный эфир без шапки и подвала. Каждый слайд сначала полностью показывается на казахском языке, затем тот же слайд — на русском, и только после этого открывается следующий. Любое касание, клик, прокрутка или нажатие клавиши открывает интерактивную новостную ленту с карточками; после заданного периода бездействия экран автоматически возвращается к эфиру. В разделе `/admin/broadcast` можно редактировать двуязычную бегущую строку, продолжительность языковых фаз и таймер возврата, добавлять ежегодные поздравления с днём рождения и загружать видеослайды MP4/WebM размером до 150 МБ. Поздравление появляется в эфире отдельным слайдом только в соответствующий день, а видео воспроизводится сначала в казахской, затем в русской фазе и после этого переключает эфир дальше.
...
Миграция `20260807090000_news` добавляет таблицу `News`, `20260807150000_news_localization` — отдельные русские и казахские поля публикаций, `20260807180000_broadcast` — элементы эфира и настройки бегущей строки, а `20260807200000_broadcast_idle` — таймер возврата к эфиру. Перед запуском обновлённого приложения примените миграции и при необходимости демонстрационные записи:
