import { BarChart3, BookOpenCheck, Boxes, ChevronRight, ClipboardList, LayoutDashboard, LogOut, Settings, ShieldCheck, Users } from 'lucide-react';
...
export default function AdminLayout() {
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><div className="admin-brand"><span>ДГД</span><div><strong>Инфокиоск</strong><small>Панель управления</small></div></div><nav>{links.filter((item) => !item.roles || item.roles.includes(user.role)).map(({ to, label, icon: Icon, end }) => <NavLink key={to} to={to} end={end}><Icon size={21} /><span>{label}</span><ChevronRight className="nav-arrow" size={17} /></NavLink>)}</nav><div className="admin-profile"><div className="admin-profile__avatar"><ShieldCheck /></div><div><strong>{user.fullName}</strong><small>{user.role}</small></div><button onClick={signOut} aria-label="Выйти"><LogOut size={21} /></button></div></aside><div className="admin-workspace"><header className="admin-topbar"><div><span>Департамент государственных доходов</span><strong>Управление информационным киоском</strong></div><a href="/" className="admin-preview-link">Открыть киоск</a></header><main className="admin-main"><Outlet /></main></div></div>;
}
