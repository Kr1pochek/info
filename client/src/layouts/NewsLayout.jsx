...
import { ArrowLeft, Eye, Languages, Newspaper } from 'lucide-react';
...
export default function NewsLayout() {
  ...
  const { fontSize, visionMode, toggleVisionMode } = useFontSize();
  ...
  if (!interactive) return <div className={`news-broadcast-entry ${accessibilityClass}`}><button type="button" className={`news-accessibility-float${visionMode ? ' active' : ''}`} onClick={toggleVisionMode} aria-pressed={visionMode}><Eye size={20} />{language === 'kz' ? 'Нашар көретіндерге' : 'Для слабовидящих'}</button><Outlet context={{ interactive, activateInteractive }} /></div>;
  return <div className={`news-shell ${accessibilityClass}`} {...activityProps}>
    <header className="news-header"><Link to="/news" className="news-brand"><span><Newspaper size={25} /></span><div><strong>{copy.brand}</strong><small>{copy.brandSubtitle}</small></div></Link><nav aria-label={language === 'kz' ? 'Жаңалықтар навигациясы' : 'Навигация новостей'}>{location.pathname !== '/news' && <Link to="/news" className="news-nav-link news-nav-back"><ArrowLeft size={19} />{copy.backToFeed}</Link>}<button type="button" className={`news-nav-link news-accessibility-button${visionMode ? ' active' : ''}`} onClick={toggleVisionMode} aria-pressed={visionMode}><Eye size={19} />{language === 'kz' ? 'Нашар көретіндерге' : 'Для слабовидящих'}</button><button type="button" className="news-nav-link news-language-switch" onClick={() => setLanguage(language === 'kz' ? 'ru' : 'kz')}><Languages size={19} />{language === 'kz' ? 'Рус' : 'Қаз'}</button></nav></header>
