...
export default function PackagesPage() {
  ...
  return <section className="page-section"><header className="page-title"><span>ДГД</span><h1>{t.servicePackages}</h1><p>{language === 'kz' ? 'Өмірлік жағдайыңызға сәйкес бөлімді таңдаңыз.' : 'Выберите раздел, подходящий под вашу жизненную ситуацию.'}</p></header><div className="package-grid">{items.map((item) => <PackageCard item={item} key={item.id} />)}</div></section>;
}
