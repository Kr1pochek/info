...
export default function CategoryPage() {
  ...
  const load = useCallback(async () => { setLoading(true); setError(''); try { const response = await api.get(`/categories/${categorySlug}`); setCategory(response.data.data); } catch (err) { setError(apiMessage(err)); } finally { setLoading(false); } }, [categorySlug]);
  useEffect(() => { load(); }, [load]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      track('SEARCH', { searchQuery: q.slice(0, 80), metadata: { language, results: services.length, scope: 'category' } });
    }, 300);
    return () => clearTimeout(timer);
  }, [language, query, services.length]);
