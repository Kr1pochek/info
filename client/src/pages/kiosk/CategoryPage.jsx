...
export default function CategoryPage() {
  ...
  const services = useMemo(() => {
    if (!category) return [];
    const q = query.trim().toLocaleLowerCase(language === 'kz' ? 'kk-KZ' : 'ru-RU'); if (!q) return category.services;
    return category.services.filter((item) => `${localized(item, 'title', language)} ${localized(item, 'shortDescription', language)} ${localized(item.category, 'title', language)}`.toLocaleLowerCase().includes(q));
  ...
  return <section className="page-section"><header className="category-hero"><div className="category-hero__icon"><AppIcon name={category.icon} size={52} /></div><div><span>{t.category}</span><h1>{localized(category, 'title', language)}</h1><p>{localized(category, 'description', language)}</p></div></header><SearchBar value={query} onChange={setQuery} />
    <div className="section-heading section-heading--compact"><div><span>{t.allServices}</span><h2>{t.servicesFound}: {services.length}</h2></div></div>{services.length ? <div className="service-grid">{services.map((service) => <ServiceCard service={service} key={service.id} />)}</div> : <EmptyState text={query ? t.noResults : t.noServices} />}
  </section>;
}
