...
import { Cake, CalendarDays, CircleDollarSign, Clock3, CloudSun, Image as ImageIcon, Newspaper, PartyPopper, RefreshCw, UserRoundCheck, WifiOff } from 'lucide-react';
...
function BroadcastScreen({ onOpenNews }) {
  ...
  useEffect(() => { if (!priorityModalOpen || !priorityNews.length) return undefined; const timer = setTimeout(() => { if (priorityIndex + 1 < priorityNews.length) setPriorityIndex((currentIndex) => currentIndex + 1); else setPriorityModalOpen(false); }, 14 * 1000); return () => clearTimeout(timer); }, [priorityIndex, priorityModalOpen, priorityNews.length]);
  useEffect(() => { if (priorityModalOpen || !priorityNews.length) return undefined; const timer = setTimeout(() => { setPriorityIndex(0); setPriorityModalOpen(true); }, 10 * 60 * 1000); return () => clearTimeout(timer); }, [priorityModalOpen, priorityNews.length]);
  ...
  return <main className={`broadcast-screen broadcast-screen--${item.kind.toLowerCase()}`}>
    ...
    <time className="broadcast-datetime" dateTime={now.toISOString()}><CalendarDays size={20} /><span>{now.toLocaleDateString(locale, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</span><Clock3 size={20} /><strong>{now.toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit' })}</strong></time>
    ...
    {priorityModalOpen && <PriorityNewsModal news={priorityNews[priorityIndex]} language={language} broadcast current={priorityIndex + 1} total={priorityNews.length} onNext={priorityIndex + 1 < priorityNews.length ? () => setPriorityIndex((currentIndex) => currentIndex + 1) : undefined} onClose={() => setPriorityModalOpen(false)} />}
  </main>;
}
