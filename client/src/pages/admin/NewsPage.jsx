...
import { CalendarDays, CalendarX2, ChevronLeft, ChevronRight, Clock3, Edit3, Eye, EyeOff, ImagePlus, Pin, Plus, Search, ShieldAlert, Trash2 } from 'lucide-react';
...
const blank = { slug: '', titleRu: '', titleKz: '', descriptionRu: '', descriptionKz: '', contentRu: '', contentKz: '', image: '', category: 'GENERAL', isPriority: false, published: false, publishAt: '', expireAt: '', sortOrder: 0 };
...
const toForm = (item) => ({ id: item.id, slug: item.slug, titleRu: item.titleRu, titleKz: item.titleKz, descriptionRu: item.descriptionRu, descriptionKz: item.descriptionKz, contentRu: item.contentRu, contentKz: item.contentKz, image: item.image || '', category: item.category, isPriority: Boolean(item.isPriority), published: item.published, publishAt: toLocalDateTime(item.publishedAt), expireAt: toLocalDateTime(item.expiresAt), sortOrder: item.sortOrder ?? 0 });
  ...
  return <form className="admin-form" onSubmit={onSubmit}>
    <div className="news-editor">
      <div className="form-grid">
        ...
        <label className={`priority-news-toggle form-grid__wide ${form.isPriority ? 'active' : ''}`}><input type="checkbox" checked={form.isPriority} onChange={(event) => setForm({ ...form, isPriority: event.target.checked, category: event.target.checked ? 'IMPORTANT' : form.category })} /><span><strong><ShieldAlert size={22} />{tr('Показывать модальным окном', 'Модальды терезеде көрсету')}</strong><small>{tr('Откроется поверх текущих новостей и материалов эфира только в заданный период.', 'Көрсетілген кезеңде ағымдағы жаңалықтар мен эфир материалдарының үстінен ашылады.')}</small></span></label>
        ...
        <label className="toggle-label form-grid__wide"><input type="checkbox" checked={form.published} onChange={(event) => setForm({ ...form, published: event.target.checked, publishAt: event.target.checked ? form.publishAt || (form.isPriority ? toLocalDateTime(new Date()) : '') : '' })} /><span>{tr('Опубликовать или запланировать выпуск', 'Жариялау немесе шығуын жоспарлау')}</span></label>
        {form.published && <><label><span>{form.isPriority ? tr('Начало показа модального окна', 'Модальды терезені көрсетудің басталуы') : tr('Дата и время публикации', 'Жариялау күні мен уақыты')}</span><input type="datetime-local" required={form.isPriority} value={form.publishAt} onChange={(event) => setForm({ ...form, publishAt: event.target.value })} /><small>{form.isPriority ? tr('С этого момента сообщение начнёт открываться поверх текущих материалов.', 'Осы сәттен бастап хабарлама ағымдағы материалдардың үстінен ашылады.') : tr('Пустое поле — публикация сразу.', 'Бос өріс — бірден жариялау.')}</small></label><label><span>{form.isPriority ? tr('Окончание показа модального окна', 'Модальды терезені көрсетудің аяқталуы') : tr('Показывать до', 'Көрсету мерзімі')}</span><input type="datetime-local" required={form.isPriority} min={form.publishAt || undefined} value={form.expireAt} onChange={(event) => setForm({ ...form, expireAt: event.target.value })} /><small>{form.isPriority ? tr('Обязательно. После этого времени окно и новость исчезнут автоматически.', 'Міндетті. Осы уақыттан кейін терезе мен жаңалық автоматты түрде жоғалады.') : tr('Необязательно. После этой даты материал исчезнет автоматически.', 'Міндетті емес. Осы күннен кейін материал автоматты түрде жоғалады.')}</small></label></>}
      </div>
      <aside className="news-editor__preview">
        <span>{tr('Предпросмотр карточки', 'Карточканы алдын ала қарау')}</span>{form.isPriority && <div className="news-priority-preview"><ShieldAlert size={16} />{tr('Модальное важное сообщение', 'Модальды маңызды хабарлама')}</div>}<strong className={newsCategoryClass(form.category)}>{newsCategoryLabel(form.category, language)}</strong>
    ...
    <div className="form-actions"><button type="button" className="admin-button admin-button--secondary" onClick={onCancel}>{tr('Отмена')}</button><button className="admin-button admin-button--primary" disabled={busy || uploading}>{busy ? tr('Сохранение…') : tr('Сохранить')}</button></div>
  </form>;
}
