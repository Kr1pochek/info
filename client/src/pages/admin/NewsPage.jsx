...
import { CalendarDays, ChevronLeft, ChevronRight, Clock3, Edit3, Eye, EyeOff, ImagePlus, Plus, Search, Trash2 } from 'lucide-react';
...
const blank = { slug: '', title: '', description: '', content: '', image: '', category: 'GENERAL', published: false, publishAt: '' };
...
const toForm = (item) => ({ id: item.id, slug: item.slug, title: item.title, description: item.description, content: item.content, image: item.image, category: item.category, published: item.published, publishAt: toLocalDateTime(item.publishedAt) });
  ...
  return <form className="admin-form" onSubmit={onSubmit}>
    <div className="news-editor">
      <div className="form-grid">
        <label className="form-grid__wide"><span>Заголовок</span><input required maxLength={240} value={form.title} onChange={(event) => { const title = event.target.value; const slugFollowsTitle = !form.slug || form.slug === createSlug(form.title); setForm({ ...form, title, slug: slugFollowsTitle ? createSlug(title) : form.slug }); }} /></label>
        ...
        <label className="form-grid__wide"><span>Краткое описание</span><textarea required maxLength={800} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label>
        <label className="form-grid__wide"><span>Полный текст</span><textarea className="news-content-input" required maxLength={30000} value={form.content} onChange={(event) => setForm({ ...form, content: event.target.value })} placeholder="Разделяйте абзацы пустой строкой" /></label>
        ...
        {form.published && <label className="form-grid__wide"><span>Дата и время публикации</span><input type="datetime-local" value={form.publishAt} onChange={(event) => setForm({ ...form, publishAt: event.target.value })} /><small>Оставьте поле пустым для немедленной публикации. Будущая дата запланирует автоматический выпуск.</small></label>}
      </div>
      <aside className="news-editor__preview">
        ...
        <p>{form.description || 'Краткое описание будет показано в общей ленте.'}</p>
      </aside>
    </div>
...
export default function NewsPage() {
  ...
  const save = async (event) => {
    ...
    const { id, publishAt, ...payload } = editing;
  ...
  return <>
    ...
    {scheduled.length > 0 && <section className="admin-schedule"><header><div><Clock3 size={20} /><span>Ближайшие публикации</span></div><strong>{scheduled.length}</strong></header><div>{scheduled.map((item) => <button onClick={() => setEditing(toForm(item))} key={item.id}><time>{new Date(item.publishedAt).toLocaleString('ru-RU', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</time><span>{item.title}</span><ChevronRight size={17} /></button>)}</div></section>}
    <div className="admin-toolbar admin-toolbar--filters"><label className="admin-search"><Search /><input value={filters.search} onChange={(event) => setFilters({ ...filters, search: event.target.value, page: 1 })} placeholder="Поиск по заголовку или описанию" /></label><select value={filters.publication} onChange={(event) => setFilters({ ...filters, publication: event.target.value, page: 1 })}><option value="">Все статусы</option><option value="live">Опубликованные</option><option value="scheduled">Запланированные</option><option value="draft">Черновики</option></select><select value={filters.category} onChange={(event) => setFilters({ ...filters, category: event.target.value, page: 1 })}>{newsCategories.map((item) => <option value={item.value} key={item.value || 'all'}>{item.label}</option>)}</select><span>{meta.total} записей</span></div>
    {error && !editing && <ErrorState title="Не удалось загрузить новости" text={error} onRetry={load} />}
    {!rows && !error ? <LoadingState /> : rows && <div className="admin-card admin-table-wrap"><table className="admin-table"><thead><tr><th>Новость</th><th>Категория</th><th>Статус</th><th>Публикация</th><th>Обновлена</th><th></th></tr></thead><tbody>{rows.map((item) => { const isScheduled = item.published && item.publishedAt && new Date(item.publishedAt) > new Date(); const live = item.published && !isScheduled; return <tr key={item.id}><td><div className="table-title news-table-title"><span>{item.image ? <img src={assetUrl(item.image)} alt="" /> : <ImagePlus />}</span><div><strong>{item.title}</strong><small>{item.author?.fullName || '—'} · {item.slug}</small></div></div></td><td><span className={newsCategoryClass(item.category)}>{newsCategoryLabel(item.category)}</span></td><td><span className={`status-pill ${isScheduled ? 'status-pill--accent' : live ? 'status-pill--success' : 'status-pill--muted'}`}>{isScheduled ? 'Запланирована' : live ? 'Опубликована' : 'Черновик'}</span></td><td>{item.publishedAt ? <span className="scheduled-date">{isScheduled && <Clock3 size={14} />}{new Date(item.publishedAt).toLocaleString('ru-RU', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span> : '—'}</td><td>{new Date(item.updatedAt).toLocaleDateString('ru-RU')}</td><td><div className="row-actions">{live && <a href={`/news/${item.slug}`} aria-label="Открыть новость"><Eye /></a>}<button onClick={() => changePublication(item)} aria-label={item.published ? 'Снять с публикации' : 'Опубликовать'}>{item.published ? <EyeOff /> : <Eye />}</button><button onClick={() => setEditing(toForm(item))} aria-label="Редактировать"><Edit3 /></button>{canDelete && <button onClick={() => setDeleting(item)} aria-label="Удалить"><Trash2 /></button>}</div></td></tr>; })}</tbody></table></div>}
    ...
    {deleting && <ConfirmDialog title="Удалить новость?" text={`Новость «${deleting.title}» будет удалена без возможности восстановления.`} onConfirm={remove} onCancel={() => setDeleting(null)} busy={busy} />}
