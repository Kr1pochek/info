...
const toForm = (item) => ({ id: item.id, titleRu: item.titleRu, titleKz: item.titleKz, descriptionRu: item.descriptionRu, descriptionKz: item.descriptionKz, contentRu: item.contentRu, contentKz: item.contentKz, image: item.image || '', category: item.isPriority ? 'VERY_IMPORTANT' : item.category, isPriority: Boolean(item.isPriority), showInBroadcast: item.isPriority ? false : Boolean(item.showInBroadcast), published: item.published, publishAt: toLocalDateTime(item.publishedAt), expireAt: toLocalDateTime(item.expiresAt), sortOrder: item.isPriority ? 0 : item.sortOrder ?? 0 });

function NewsForm({ form, setForm, onSubmit, onCancel, onUpload, uploading, busy, error }) {
  const { language, tr } = useAdminI18n();
  const changeCategory = (category) => {
    const isPriority = category === 'VERY_IMPORTANT';
    setForm({
      ...form,
      category,
      isPriority,
      ...(isPriority ? { showInBroadcast: false, sortOrder: 0, publishAt: form.publishAt || toLocalDateTime(new Date()) } : {}),
    });
  };
  ...
  return <form className="admin-form" onSubmit={onSubmit}>
    <div className="news-editor">
      <div className="form-grid">
        ...
        <label className="form-grid__wide"><span>{tr('Категория', 'Санат')}</span><select value={form.category} onChange={(event) => changeCategory(event.target.value)}>{newsCategories.slice(1).map((item) => <option value={item.value} key={item.value}>{language === 'kz' ? item.labelKz : item.label}</option>)}</select>{form.isPriority && <small>{tr('Категория «Очень важная новость» автоматически включает показ в модальном окне.', '«Өте маңызды жаңалық» санаты модальды терезеде көрсетуді автоматты түрде қосады.')}</small>}</label>
        ...
        <fieldset className="broadcast-library-field form-grid__wide">
          <legend>{tr('Готовая обложка из библиотеки', 'Кітапханадағы дайын мұқаба')}</legend>
          <p>{tr('Выберите локальное изображение для новости — оно доступно в закрытой сети без интернета.', 'Жаңалық үшін жергілікті суретті таңдаңыз — ол жабық желіде интернетсіз қолжетімді.')}</p>
          <MediaLibraryPicker value={form.image} onChange={(image) => setForm({ ...form, image })} language={language} />
        </fieldset>
        <label className="form-grid__wide"><span>{tr('Своя обложка — необязательно', 'Өз мұқабаңыз — міндетті емес')}</span><span className="image-upload-control"><ImagePlus size={20} />{uploading ? tr('Загрузка…') : form.image ? tr('Заменить изображение', 'Суретті ауыстыру') : tr('Загрузить изображение', 'Сурет жүктеу')}<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={onUpload} disabled={uploading} /></span><small>{tr('Можно выбрать готовую обложку выше, загрузить свою или опубликовать без изображения. JPG, PNG, WebP или GIF, до 5 МБ.', 'Жоғарыдан дайын мұқабаны таңдауға, өз суретіңізді жүктеуге немесе суретсіз жариялауға болады. JPG, PNG, WebP немесе GIF, 5 МБ дейін.')}</small>{form.image && <button type="button" className="news-image-remove" onClick={() => setForm({ ...form, image: '' })}><Trash2 size={15} />{tr('Убрать изображение', 'Суретті алып тастау')}</button>}</label>
...
export default function NewsPage() {
  ...
  const changePublication = async (item) => {
    if (!item.published && item.isPriority && (!item.expiresAt || new Date(item.expiresAt) <= new Date())) { setEditing(toForm(item)); setToast({ type: 'error', message: 'Сначала задайте новый период показа очень важной новости' }); return; }
  ...
  return <>
    <AdminPageHeader eyebrow="Контент" eyebrowKz="Контент" title="Новости" titleKz="Жаңалықтар" description="Создание, редактирование и публикация материалов корпоративной ленты" descriptionKz="Корпоративтік таспа материалдарын жасау, өңдеу және жариялау" actions={<><button className="admin-button admin-button--priority" onClick={() => setEditing({ ...blank, isPriority: true, category: 'VERY_IMPORTANT', published: true, publishAt: toLocalDateTime(new Date()) })}><ShieldAlert size={19} />{tr('Очень важная новость', 'Өте маңызды жаңалық')}</button><button className="admin-button admin-button--primary" onClick={() => setEditing({ ...blank })}><Plus size={19} />{tr('Новая новость', 'Жаңа жаңалық')}</button></>} />
    ...
    {editing && <Modal title={editing.id ? 'Редактирование новости' : editing.isPriority ? 'Очень важная новость' : 'Новая новость'} titleKz={editing.id ? 'Жаңалықты өңдеу' : editing.isPriority ? 'Өте маңызды жаңалық' : 'Жаңа жаңалық'} onClose={() => setEditing(null)} wide><NewsForm form={editing} setForm={setEditing} onSubmit={save} onCancel={() => setEditing(null)} onUpload={upload} uploading={uploading} busy={busy} error={error} /></Modal>}
