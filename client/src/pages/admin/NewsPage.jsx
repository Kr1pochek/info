  ...
  return <form className="admin-form" onSubmit={onSubmit}>
    <div className="news-editor">
      ...
      <aside className="news-editor__preview">
        <span>{tr('Предпросмотр карточки', 'Карточканы алдын ала қарау')}</span><div className={`news-placement-preview${form.showInBroadcast ? ' is-broadcast' : ''}`}>{form.showInBroadcast ? <MonitorPlay size={16} /> : <Newspaper size={16} />}{form.showInBroadcast ? tr('Лента + эфир', 'Таспа + эфир') : tr('Только лента', 'Тек таспа')}</div>{form.isPriority && <div className="news-priority-preview"><ShieldAlert size={16} />{tr('Важная новость во всплывающем окне', 'Қалқымалы терезедегі маңызды жаңалық')}</div>}<strong className={newsCategoryClass(form.category)}>{newsCategoryLabel(form.category, language)}</strong>
