...
export default function NewsCard({ news }) {
  ...
  return <article className="news-card">
    ...
    <div className="news-card__body"><div className="news-card__meta"><span className={newsCategoryClass(item.category)}>{newsCategoryLabel(item.category, language)}</span><time dateTime={item.publishedAt || item.createdAt}><CalendarDays size={15} />{newsDate(item, language)}</time></div><h2><Link to={`/news/${item.slug}`}>{item.title}</Link></h2><p>{item.description}</p><Link to={`/news/${item.slug}`} className="news-card__link">{copy.readFull} <ArrowRight size={18} /></Link></div>
  </article>;
}
