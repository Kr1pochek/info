...
export default function InactivityGuard({ children }) {
  ...
  const resetSession = useCallback((eventType = 'SESSION_TIMEOUT') => {
    sessionStarted.current = false; lastActivity.current = null; setRemaining(null); resetLanguage(); resetFontSize(); endSession(eventType); navigate('/kiosk', { replace: true }); window.scrollTo({ top: 0, behavior: 'auto' });
  ...
  useEffect(() => {
    ...
    window.addEventListener('dgd-kiosk-session-ended', sessionEnded);
    ...
    return () => { clearInterval(timer); window.removeEventListener('dgd-kiosk-session-ended', sessionEnded); kioskActivityEvents.forEach((event) => window.removeEventListener(event, activity)); };
